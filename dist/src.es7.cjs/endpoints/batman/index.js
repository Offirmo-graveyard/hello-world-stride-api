...
const APP_ID = 'stride-app-batman';
async function factory(dependencies = {}) {
    ...
    function getRandomAssistanceMsg({ user }) {
        ...
        const content = random_1.Random.pick(rng, [
            [
                { type: "text", text: `When you face your struggles, ` },
                mention,
                { type: "text", text: ` , you overcome them.` },
            ],
            [
                { type: "text", text: `Tell your friends, tell all your friends I am with ` },
                mention,
            ],
            [
                { type: "text", text: `It's not who you are underneath, ` },
                mention,
                { type: "text", text: ` , it's what you do that defines you.` },
            ],
        ]);
        return {
            version: 1,
    ...
    app.post('/on-any-message', (req, res, next) => {
        ...
        (async function process() {
            ...
            if (txt.includes('joker')) {
                stride.sendTextMessageDirectedToUser({
                    cloudId,
                    conversationId,
                    userId: senderId,
                    text: `The joker? Where is that scum?`
                });
            }
            res.sendStatus(204);
        })()
